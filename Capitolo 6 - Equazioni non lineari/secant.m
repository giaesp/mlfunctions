...
%
% Prototipo: [x,convg,iter,err] = secant(f,x1,x2,int_toll,zero_toll,max_iter,v_mode,g_mode)
% Input:     La funzione f
...
function [x,convg,iter,err] = secant(f,x1,x2,int_toll,zero_toll,max_iter,v_mode,g_mode)
...
for iter = 1 : max_iter
   ...
   if (iter > 1)
      convg = log(abs(x2 - x3)/abs(x1 - x3))/log(abs(x1 - x3)/abs(x0 - x3));
   end
