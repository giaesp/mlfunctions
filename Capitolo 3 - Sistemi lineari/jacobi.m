...
% Note: Controllare la convergenza del metodo di Jacobi utilizzando l'apposita
%       funzione jtest (>>help jtest per ulteriori dettagli).
%
% Prototipo: [x,iter,err] = jacobi(A,b,x,toll,max_iter,verbose_mode,graphic_mode)
...
if (graphic_mode == 1)
   hold on;
   grid on;
   title(' Grafico degli errori - Metodo di Jacobi');
   xlabel('Iterazioni');
...
while (iter < max_iter) & (error > toll)
   ...
   for i = 2 : n - 1
      x(i) = (b(i) - A(i,1:i-1) * xk(1:i-1) - A(i,i+1:n) * xk(i+1:n))/A(i,i);
   end
   x(n) = (b(n) - A(n,1:n-1) * xk(1:n-1)) / A(n,n);
